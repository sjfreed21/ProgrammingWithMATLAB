function samples = fsong( score )
  ...
  %  - duration specifies the length in seconds. THE TONE
  %    GENERATED WILL ALSO APPLY A FADE-OUT ACROSS THIS 
  %    DURATION.
...
function rv = tone(duration, freq)
  % Generates the samples FADING sine wave for the given 'freq'
  % and 'duration'.  The sampling rate is 8192 Hz.
  sampleTimes = (0:duration*8192-1)/8192;
  rv = sin(2*pi*freq*sampleTimes) * .999 .* exp(-10*sampleTimes); % scale
end
